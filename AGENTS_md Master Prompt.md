...
2.  **No Forced Overwrite:** 기존 `AGENTS.md`를 무조건 덮어쓰지 마십시오. 기존 내용을 보존한 상태에서 **diff 기반으로 갱신(Update)** 하십시오.
3.  **Versioned Change Policy:** `AGENTS.md` 변경 시 변경 이유를 PR/커밋 메시지에 명시하고, 필요 시 `AGENTS_CHANGELOG.md`에 변경 이력을 기록하십시오.
